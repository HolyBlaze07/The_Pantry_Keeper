...
function hexToRgb(hex: string): [number, number, number] {
  ...
  const numericHex = parseInt(value.slice(0, 6), 16)
  return [
    ((numericHex >> 16) & 255) / 255,
    ((numericHex >> 8) & 255) / 255,
    (numericHex & 255) / 255,
  ]
}
...
export default function FaultyTerminal({
  ...
  const classNames = ['faulty-terminal-container', className]
    .filter(Boolean)
    .join(' ')

  return <div ref={containerRef} className={classNames} style={style} {...rest} />
}